...
async function seedGames(db: Database, count: number): Promise<{
    categoryId: number;
    publisherId: number;
}> {
...
describe('games data-access helpers', () => {
    ...
    it('filters games by category and publisher', async () => {
        const { categoryId, publisherId } = await seedGames(db, 3);
    ...
    it('matches games from any selected category and preserves title ordering', async () => {
    ...
    it('returns an empty collection when filters match no games', async () => {
        ...
        await expect(getGames(db, { publisherId: 99999 })).resolves.toEqual([]);
    });
